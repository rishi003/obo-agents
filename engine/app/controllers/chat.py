...
@router.post(
    ...
    responses={400: {"detail": "User not found"}},
)
def create_chat(chat: ChatIn):
    ...
    db_participant = Participant(
        id=str(uuid.uuid4()), userId=chat.user_id, chatId=db_chat.id
    )
...
@router.get(
    "/get", response_model=List[ChatOut], responses={404: {"detail": "No chats found"}}
)
def get_chat(user_id: str):
    ...
    chats = (
        ...
        .join(Chat, Participant.chatId == Chat.id)
        .filter(Participant.userId == user_id)
        .all()
    )

    chats = [ChatOut(id=chat.id, name=chat.name) for chat in chats]
...
def create_message(message: MessageIn):
    ...
    if not docs:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No documents found for user"
        )

    participant = (
        db.session.query(Participant)
        .filter(
            Participant.chatId == message.chat_id,
            and_(Participant.userId == message.user_id),
        )
        .first()
    )
    if not participant:
        db_participant = Participant(
            id=str(uuid.uuid4()), userId=message.user_id, chatId=message.chat_id
        )
        db.session.add(db_participant)
        db.session.commit()
    db_message = Message(
